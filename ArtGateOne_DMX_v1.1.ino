...
void setup()
{
  ...
  else if (EEPROM.read(512) == 1) { //uruchomienie ethernet w zależności od ustawień dhcp
    IPAddress ip(EEPROM.read(513), EEPROM.read(514), EEPROM.read(515), EEPROM.read(516));
...
void loop()
{
  ...
  if (packetSize == 14) {
    ...
    buf[188] = 0x00; // SwIn 2
    buf[189] = 0x00; // SwIn 3
    buf[190] = intU; // SwOut 0//ODBIERA UNIVERSE NR intU;
    ...
    buf[198] = 0x00; // Spare
    buf[199] = 0x00; // Spare
    buf[200] = 0x00; // Style 0x00 StNode A DMX to / from Art-Net device
    // MAC ADDRESS
